...
function PredictionBit28() {
  const { siteSettings } = useAuth();
  const navigate = useNavigate();
  const [selectedAlgorithm, setSelectedAlgorithm] = useState(null);
  
  const { data: predictions, loading: predictionsLoading, refetch: refetchPredictions } = usePredictions('bit28');
  const { data: algorithms, loading: algorithmsLoading, refetch: refetchAlgorithms } = useAlgorithmCompare('bit28');
  ...
  return (
    <>
      <Helmet>
        <title>{String('比特28预测 - ' + (siteSettings?.site_name ?? '大海团队'))}</title>
        <meta name="description" content="比特28算法预测与准确率统计" />
      </Helmet>
      
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 p-4 pb-24">
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          <Button 
            ...
            variant="ghost" 
            size="sm"
            className="mb-2 text-orange-600 hover:text-orange-700 hover:bg-orange-100"
          >
          ...
          {/* 顶部标题横幅 */}
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-orange-600 via-amber-600 to-yellow-600 p-6 text-white shadow-xl">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-orange-100 text-sm">基于最新100条已验证数据的算法准确率统计</p>
                </div>
                <Button 
                ...
                return (
                  <Card
                    ...
                    className={`cursor-pointer transition-all hover:shadow-xl border-2 ${
                      isSelected ? 'ring-2 ring-offset-2 ring-orange-500 scale-105' : color.bg
                    }`}
...
export default PredictionBit28;
